...
output "endpoint_internal" {
  ...
  value = [
    ...
    format("%s:3306", aws_db_instance.primary.address) :
    format("%s.%s:3306", aws_service_discovery_service.primary[0].name, var.infrastructure.domain_suffix)
  ]
}
...
output "endpoint_internal_readonly" {
  ...
  value = local.architecture == "replication" ? flatten([
    var.infrastructure.domain_suffix == null ?
    formatlist("%s:3306", aws_db_instance.secondary[*].address) :
    format("%s.%s:3306", aws_service_discovery_service.secondary[0].name, var.infrastructure.domain_suffix)
  ]) : []
}
