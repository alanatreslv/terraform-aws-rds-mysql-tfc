...
output "endpoint_internal" {
  ...
  value       = [format("%s.%s:3306", aws_service_discovery_service.primary.name, var.infrastructure.domain_suffix)]
}
...
output "endpoint_internal_readonly" {
  ...
  value       = local.architecture == "replication" ? [format("%s.%s:3306", aws_service_discovery_service.secondary[0].name, var.infrastructure.domain_suffix)] : []
}
